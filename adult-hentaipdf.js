...
const handler = async (m, {conn, text, usedPrefix, command, args}) => {
  if (!db.data.chats[m.chat].modohorny && m.isGroup) throw '*『✦』Los comandos para adultos estan prohibidos por ahora.*';
  if (!text) throw `*『✦』Ingrese el nombre un anime H para buscarlo. Por ejemplo: ${usedPrefix + command} hinata*`;
  try {
    m.reply(global.wait);
