...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  if (!text) return m.reply(`*『✦』Use el comando de la siguiente manera:*\n\n*${usedPrefix + command}* que tal @${m.sender.split`@`[0]} a`, null, {mentions: [m.sender]});
  const cm = copy(m);
  let who;
  if (text.includes('@0')) who = '0@s.whatsapp.net';
  else if (m.isGroup) who = cm.participant = m.mentionedJid[0];
  else who = m.chat;
  if (!who) return m.reply(`*『✦』Use el comando de la siguiente manera:*\n\n*${usedPrefix + command}* que tal @${m.sender.split`@`[0]} a`, null, {mentions: [m.sender]});
  cm.key.fromMe = false;
