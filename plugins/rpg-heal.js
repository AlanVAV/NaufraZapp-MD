...
const handler = async (m, {conn, args, usedPrefix, __dirname}) => {
  ...
  if (user.health >= 100) return conn.reply(m.chat, `*『✦』Tu salud esta llena.*\n\nSALUD: ${user.health}`, m);
...
[`🏕️ 𝙰𝚅𝙴𝙽𝚃𝚄𝚁𝙰𝚁`, `${usedPrefix}adventure`]], m)*/
  ...
  if (user.potion < count) {
    return conn.reply(m.chat, `*『✦』No tienes pociones.*\n\nnecesutas: ${count - user.potion} de pocion para curarte.

Salud: ${user.health}
Pocion: ${user.potion}

*『✦』Compra pocion para recuperarte.*`, m);
  }
